...
async def contract_generation_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        late_data = sup_client.table("Leads").select("role", "phone").eq("lead_id", lead_id).execute().data[0]

        job_title = late_data["role"]
        phone = str(late_data["phone"])

        lead_contact = await create_opensign_contact.ainvoke({"email": lead_email,"name": lead_name,"job_title": job_title,"company": company_name,"phone": phone})

        lead_signer_id = lead_contact["objectId"]

        logger.info(f"The lead has been successfully added as a contact in OpenSign ({lead_signer_id}). It now proceeds to generate the contract...")
        ...
        logger.info(f"Contact has been found, and the rep_signer_id is {rep_signer_id}")

        contract_pdf = generate_contract_pdf(lead_name, lead_email, rep_name, rep_email, company_name, deal_size)
        logger.info("The contract has been successfully generated by the service script.")
        ...
        logger.info("The contract has been successfully uploaded to OpenSign.")
        ...
        if success:
            logger.info(f"Contract '{contract_id}' has been sent successfully for {lead_name} with {lead_email}")
        else:
        ...
        doc = await get_opensign_document.ainvoke({"document_id": contract_id})
        file_url = doc["file"]
        ...
        sup_client.table("contracts").insert({
                "lead_id": lead_id,
        ...
        logger.info("Contract created successfully in the database.")
        return state
